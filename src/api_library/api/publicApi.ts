...
export class PublicApi {
     ...
     * @param typeLimit Specific type of message used to limit the results
     * @param search Regex string to use to restrict the results via search
     * @param perPage Number of audit log records to retrieve
     * @param page Skip to page number, based on per_page value
     * @param skip Alternative to page, will fully define start index of results rather than calculating based on page/per_page values
     */
    public async getChangeLogs (typeLimit?: string, search?: string, perPage?: number, page?: number, skip?: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Array<InlineResponse200>;  }> {
        const localVarPath = this.basePath + '/changelogs';
        ...
        if (typeLimit !== undefined) {
            localVarQueryParameters['typeLimit'] = ObjectSerializer.serialize(typeLimit, "string");
        }
